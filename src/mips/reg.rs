...
#[inline]
pub unsafe fn read<T>() -> u32 where T: GeneralPurposeRegister {
    let value: u32;
    asm!("move {0}, ${1}", out(reg) value, const T::INDEX);
    value
}

#[inline]
pub unsafe fn write<T>(value: u32) where T: GeneralPurposeRegister {
    asm!("move ${0}, {1}", const T::INDEX, in(reg) value);
}
